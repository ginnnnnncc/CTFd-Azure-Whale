...
from .models import DynamicDockerChallenge, DynamicValueDockerChallenge, ResourceGroup
...
def load(app):
    ...
    @page_blueprint.route('/admin/resourcegroups', methods=['GET'])
    @admins_only
    def admin_list_resource_groups():
        resource_groups = ResourceGroup.query.all()
        return render_template('resource_groups.html', groups=resource_groups)
    ...
    def admin_list_containers():
        ...
        containers = DBUtils.get_all_container_page(page_start, page_end)
    ...
    def add_container():
        ...
        cpu = dynamic_docker_challenge.cpu
        resource_group = DBUtils.get_available_resource_group(cpu)
        if resource_group is None:
            return json.dumps({'success': False, 'msg': 'No available resource group.'})
        
        flag = dynamic_docker_challenge.flag.replace("[random]", uuid.uuid4().hex[:8])
        ControlUtil.add_container(user_id=user_id, challenge_id=challenge_id, flag=flag,
                                  resource_group_name=resource_group.name,
                                  container_basename=dynamic_docker_challenge.container_name,
                                  yaml=dynamic_docker_challenge.yaml,
                                  cpu=cpu)
    ...
    def list_container():
        ...
        if data is not None:
            ...
            dynamic_docker_challenge = DynamicDockerChallenge.query \
                ...
                .first_or_404()
            resource_group = ResourceGroup.query.filter(ResourceGroup.name == data.resource_group_name).first()
            return json.dumps({'success': True, 'status': data.status, 'ip': data.ip,
                                'port': dynamic_docker_challenge.port, 'region': resource_group.region,
